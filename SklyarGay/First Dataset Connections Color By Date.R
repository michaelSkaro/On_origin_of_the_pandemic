...
plot_my_connection <- function(dep_lon,dep_lat,arr_lon,arr_lat, color){
  ...
  lines(subset(inter,lon>=0),col=c(color), lwd=1.5)
  lines(subset(inter,lon<0),col=c(color), lwd=1.5)
}
